    ...
    .then((r) => {
      if (r.length) {
        ...
        if (r[0]["pass"] === pass) {
          ...
          baseList.users
            .update({ name: params.name }, { lastLoginTime: Date.now() })
            .then((raw) => {
              console.log(raw);
            });
...
const signUp = (params, callback) => {
  ...
  baseList.users.find({ name: params.name }).then((r) => {
    if (r.length) {
      ...
      function fineOneAccountBase(createfun) {
        // 查找code
        let rand = Math.random();
        baseList.accountBase.findOneAndUpdate(
          { type: "1", status: "0", random: { $gte: rand } },
          { status: "1" },
          (err, doc) => {
            if (err) {
              console.log(err);
              ...
              if (!doc) {
                baseList.accountBase.findOneAndUpdate(
                  { type: "1", status: "0", random: { $lt: rand } },
                  { status: "1" },
                  (err, doc) => {
                    if (err) {
                      console.log(err);
    ...
    baseList.users.find({ name: params.key }).then((r) => {
      if (r.length) {
        let response = {
          ...
          photo: r[0].photo,
          bubble: r[0].bubble,
          chatTheme: r[0].chatTheme,
          projectTheme: r[0].projectTheme,
          wallpaper: r[0].wallpaper,
          nickname: r[0].nickname,
          signature: r[0].signature,
          id: r[0]._id,
          conversationsList: r[0].conversationsList,
          code: r[0].code,
          emoji: r[0].emoji,
          chatColor: r[0].chatColor,
          province: r[0].province,
          city: r[0].city,
          town: r[0].town,
          bgOpa: r[0].bgOpa,
...
const addConversitionList = (userName, params, callback) => {
  // 添加会话
  baseList.users
    .update({ name: userName }, { $push: { conversationsList: params } })
    .then((raw) => {
      if (raw.nModified > 0) {
        callback({ code: 0 });
      } else {
...
const removeConversitionList = (userName, params, callback) => {
  ...
  baseList.users
    .update(
      { name: userName },
      { $pull: { conversationsList: { id: params.id } } }
    )
    .then((raw) => {
      if (raw.nModified > 0) {
        callback({ code: 0 });
...
const huntFriends = (params, callback) => {
  ...
  let arr = [];
  params.type === "2"
    ? (arr = [{ nickname: { $regex: key, $options: "$i" } }])
    : (arr = [
        {
          code: {
            $regex: key,
