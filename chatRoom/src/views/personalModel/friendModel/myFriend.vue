<template>
  <div class="vchat-myFriend vchat-noUser">
    ...
    <div class="vchat-myFriend-container">
      ...
      <div
        class="vchat-linkman-container"
        :class="{active: showList.indexOf('mine') > -1}"
    ...
    >
      <v-dropdown-item
        slot-scope="{command}"
        @dropdownClick="handleConversitionList(command)"
        slot="dropdown"
      >
...
export default {
  ...
  computed: {
    ...
    addOrDel() {
      return this.conversationsList.filter(
        (v) => v.id === this.currFriend.roomid
      ).length;
    },
  },
  methods: {
