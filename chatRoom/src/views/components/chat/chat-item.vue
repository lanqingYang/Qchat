<template>
  <div class="vchat-item">
    <div class="vchat-item-header">
      <span
        :class="{active: currNav === v.id}"
        v-for="(v, i) in navList"
        :key="i"
        @click="setCurrNav(v.id)"
        v-if="v.type.indexOf(currSation.type) > -1"
      >{{v.name}}</span>
    </div>
    <div
      class="vchat-item-container"
      v-show="currNav === 0"
    >
      <div class="container-chat">
        ...
        <div class="chat-send">
            ...
            <span class="tool-item">
              ...
              >
                <v-icon
                  name="wenjian2"
                  :color="user.chatColor"
                  @clickIcon="uplaodVisible.f = !uplaodVisible.f"
                  title="选择文件"
                ></v-icon>
              </v-upload-popover>
            </span>
          ...
          <div class="enter">
            <button
              class="vchat-button-mini info"
              @click="clear"
            >清空</button>
            <button
              class="vchat-button-mini"
              @click="send(false)"
            >发送</button>
          </div>
        </div>
      ...
      <div
        class="container-handel"
        v-if="currSation.type === 'group'"
      >
        <div class="handel-notice">
          ...
          <ul>
            ...
            ><a
                ...
                target="_blank"
                style="color: #fff"
              >说点什么吧 <i :style="{color: 'red', fontStyle: 'normal'}">❤❤❤</i></a></li>
          </ul>
        </div>
        <div class="handel-member">
          ...
          <ul>
            <li
              v-for="v in groupUserList"
              :key="v.userId['_id']"
            >
              <a
                class="vchat-photo"
                :class="{lineOf: !v.status}"
              >
                <img
                  :src="IMG_URL + v.userId.photo"
                  alt=""
                >
              </a>
            ...
            <li>
              <p
                class="loadmore"
                v-if="groupUsers.length > groupUserList.length"
                @click="loadmore"
              >
                <v-icon
    ...
    >
      <message-log
        :currSation="currSation"
        :currNav="currNav"
        @lookPhoto="lookPhoto"
      ></message-log>
    </div>
    <v-photo-swipe
      :visible="photoSwipeFlag"
      @close="photoSwipeFlag = false"
      :url="photoSwipeUrl"
    ></v-photo-swipe>
  </div>
</template>
